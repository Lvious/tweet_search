...
def clustering_offline():
	...
	for t in range(24*6)[:-1]:	
		...
		while if_continue:
			...
			query = db.fk.find({'tweet.created_at':{'$gte':time_internals[t],'$lt':time_internals[t+1]},'class.1':{'$gte':0.5},'cluster':None},{'_id':1,'tweet.standard_text':1,'tweet.hashtags':1,'tweet.created_at':1}).sort('_id',1).limit(1000)
...
def clustering():
	query = db.fk.find({'class.1':{'$gte':0.5},'cluster':None},{'_id':1,'tweet.standard_text':1,'tweet.hashtags':1,'tweet.created_at':1}).sort('_id',1).limit(1000)
	ids = []
	texts = []
	hashtags = []
	dates = []
...
if __name__ == '__main__':
	print 'fk_clustering_worker start!'
	# clustering_offline()
	while True:
		queue = r.lpop('task:fk:clustering')
		if queue:
			print 'fk_clustering_worker process!'
			clustering()
		print 'fk_clustering_worker wait!'
		time.sleep(1)