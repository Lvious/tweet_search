...
def classify():
	query = db.fk.find({'class':None},{'_id':1,'tweet.raw_text':1})
	ids = []
	texts = []
	for i in query:
		ids.append(i['_id'])
...
if __name__ == '__main__':
	print 'fk_classify_worker start!'
	while True:
		queue = r.lpop('task:fk:classify')
		if queue:
			print 'fk_classify_worker process!'
			classify()
			message = json.loads(queue)
			print message
			if message['is_last']:
				r.rpush('task:fk:clustering',json.dumps(message))
		print 'fk_classify_worker wait!'
		time.sleep(1)
